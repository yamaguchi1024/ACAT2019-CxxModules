...
\begin{figure}[!h]
  ...
  \begin{tikzpicture}[outer sep=0.05cm, node distance=0.8cm, scale=0.7, transform shape]
    ...
    \node[model, fill=my_purple, name=virtmap, above = 1cm of include] (virtmap) {VMOF};
  ...
  \end{tikzpicture}
  \caption{Modulemap directory physical vs virtual structure. Blue rectangles describe the physical directories, purple rectangles physical files. Black thick arrows denote physical files. Dashed lines show the virtual file system relationship.}
  \label{fig:modulemap}
\end{figure}

The process of modularization subsumes external dependencies such as \textit{libc}, \textit{libstdc++} and \textit{libxml}. Upon modularizing their header files, we need to generate their modulemaps as those libraries do not have their own modulemaps. Moreover, as shown in Listing \ref{list:modulemap}, modulemaps need to know the location of headers. However, system headers are located in non-writable areas and modulemaps are impossible to be placed in order to access the headers with relative paths, unless super user privileges are granted.
